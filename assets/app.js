...
async function launchOpenLabelPrint(l, {
  ...
  const expDate = expiryDate || l.expiry_date || '';
    ...
    (async () => {
      try {
        const result = await sb.rpc('fn_log_open_label_print_v2', {
          ...
          p_opened_on:openDate,
          p_expiry_date:expDate || null,
        ...
        if (l.lot_id) {
          try { await sb.rpc('fn_log_open_label_print', {p_lot_id:l.lot_id,p_opened_on:openDate}); } catch (_) {}
        }
      }
    })();
    return true;
...
async function printOpenLabelIssue(transactionId) {
  ...
  const popup = preparePrintPopup('กำลังเตรียมสติ๊กเกอร์วันเปิดใช้', 'ระบบกำลังใส่ข้อมูลจากรายการนำออก…');
  if (!popup) return toast('เบราว์เซอร์บล็อกหน้าพิมพ์ กรุณาอนุญาต Pop-up ของเว็บไซต์นี้', true);
  const ok = await launchOpenLabelPrint(row, {
    popup,
    openedOn:row.issue_date,
    openedBy:row.issued_by_name || row.issued_by_email || profile?.display_name,
...
async function renderOpenLabelCreate() {
  ...
  const today = dateInputValue(new Date());
  page.innerHTML = `<div class="page-head"><div><p class="eyebrow">Manual label generator</p><h2>สร้างสติ๊กเกอร์วันเปิดใช้</h2><p class="muted small">ใช้เมื่อต้องการสร้างสติ๊กเกอร์เอง เช่น น้ำเกลือหรือวัสดุที่ไม่ได้มาจากรายการนำออกล่าสุด</p></div><button class="mini secondary" data-route="open-labels">${icon('history')} กลับไปรายการนำออก</button></div>
  <section class="card open-label-generator-card"><div class="open-label-generator-head">${icon('print')}<div><h3>ข้อมูลบนสติ๊กเกอร์</h3><p>ขนาด 25 × 20 mm ตามแบบ FM-CNPL-034 Rev.00</p></div></div><form id="openLabelGeneratorForm" class="form-grid open-label-generator-form">
    ...
    <label>วันเปิดใช้<input id="openLabelOpenedOn" type="date" value="${today}" required></label>
    <label>วันหมดอายุ<input id="openLabelExpiry" type="date" required></label>
    ...
    <button class="primary wide-action" type="submit">${icon('print')} สร้างและพิมพ์สติ๊กเกอร์</button>
  </form></section>`;
  setupMaterialCombobox('openLabelMaterial',materials,{maxResults:20});
  $('#openLabelGeneratorForm')?.addEventListener('submit', async e => {
    ...
    const openedOn = $('#openLabelOpenedOn').value;
    const expiryDate = $('#openLabelExpiry').value;
    ...
    if (!openedOn || !expiryDate) return toast('กรุณาระบุวันเปิดใช้และวันหมดอายุ', true);
    if (expiryDate < openedOn) return toast('วันหมดอายุต้องไม่ก่อนวันเปิดใช้', true);
