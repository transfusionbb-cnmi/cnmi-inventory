...
function expiryAlertDays(x) {
  const direct = Number(x?.expiry_alert_days);
  if ([7, 14, 30, 240].includes(direct)) return direct;
  return Number(x?.expiry_alert_months) === 8 ? 240 : DEFAULT_EXPIRY_ALERT_DAYS;
}

function expiryAlertLabel(x) {
  const days = expiryAlertDays(x);
  return days === 7 ? '1 สัปดาห์' : days === 14 ? '2 สัปดาห์' : days === 240 ? '8 เดือน' : '1 เดือน';
}
...
function isNearExpiryDate(value, days = DEFAULT_EXPIRY_ALERT_DAYS) {
  if (!value) return false;
  const expiry = new Date(`${value}T00:00:00`);
  if (Number.isNaN(expiry.getTime())) return false;
  const today = todayStart();
  return expiry >= today && expiry <= addCalendarDays(today, days);
}

function isNearExpiryItem(x) {
  if (!x || isExpired(x)) return false;
  const thresholdDays = expiryAlertDays(x);
  if (x.expiry_date) return isNearExpiryDate(x.expiry_date, thresholdDays);
  const days = Number(x.days_to_expiry);
  return Number.isFinite(days) && days >= 0 && days <= thresholdDays;
}
...
async function globalClick(e) {
  ...
  if (openLabelPrint) {
    e.preventDefault();
    printOpenLabelIssue(openLabelPrint.dataset.openLabelPrint);
    return;
  }
  ...
  const viewCheck = e.target.closest('[data-view-check]');
  if (viewCheck) {
    e.preventDefault();
    openWeeklyResult(viewCheck.dataset.viewCheck);
    return;
  }
...
async function navigate(r, options = {}) {
  ...
  try {
    ...
    else if (r === 'my-stock' || r === 'assisted-stock') await renderMyStock(options.tab || myStockTab, null, myStockScope);
...
function groupByOwner(summaryRows = []) {
  ...
  summaryRows.forEach(row => {
    ...
    if (row.needs_reorder) {
      if (!['MONTHLY','NONE'].includes(row.alert_mode || 'MINIMUM') && Boolean(row.needs_reorder) && Number(row.total_balance || 0) <= 0) item.out_count += 1;
...
function buildMaterialGroups(summaries = [], lots = []) {
  ...
  lots.forEach(l => {
    if (!map.has(l.material_code)) map.set(l.material_code, {
      ...
      nearest_expiry:null, expired_pending_lots:0, expired_pending_balance:0, alert_mode:l.alert_mode || 'MINIMUM', reorder_day:l.reorder_day || 1, expiry_alert_days:expiryAlertDays(l), needs_reorder:false, lots:[]
    });
  ...
  return [...map.values()].map(g => {
    ...
    return {...g,
      ...
      alert_mode:g.alert_mode || 'MINIMUM', reorder_day:Number(g.reorder_day || 1), expiry_alert_days:expiryAlertDays(g), needs_reorder:Boolean(g.needs_reorder),
...
function materialGroupStatus(g) {
  ...
  if (g.alert_mode === 'MONTHLY') {
    ...
    if (g.alert_mode === 'MINIMUM' && g.total_balance <= g.min_qty) return {key:'low',label:'ถึงหรือต่ำกว่าขั้นต่ำ',badge:'warn'};
  }
  if (isNearExpiryDate(g.nearest_expiry, expiryAlertDays(g))) return {key:'expiry',label:`ใกล้หมดอายุ ≤ ${expiryAlertLabel(g)}`,badge:'warn'};
  return {key:'positive',label:'คงเหลือปกติ',badge:'ok'};
}

function materialStockCard(g) {
  const st = materialGroupStatus(g);
  const nearExpiry = isNearExpiryDate(g.nearest_expiry, expiryAlertDays(g));
...
async function renderMyStock(tab = 'overview', selectedOwner = null, scope = 'primary') {
  myStockScope = scope === 'assistant' ? 'assistant' : 'primary';
  const allowedTabs = myStockScope === 'assistant' ? ['overview','reorder'] : ['overview','reorder','settings'];
  myStockTab = allowedTabs.includes(tab) ? tab : 'overview';
  const adminMode = isAdminMode();
  const {data:allRows,error} = await sb.from('v_inventory_summary').select('*').order('material_name');
  if (error) throw error;
  const rows = allRows || [];
  const ownerEntries = [...new Map(rows.map(row => myStockScope === 'assistant'
  ...
  const scopeTitle = myStockScope === 'assistant' ? 'สต๊อกที่ฉันช่วยดูแล' : 'สต๊อกที่ฉันดูแล';
  const adminScopeTitle = myStockScope === 'assistant' ? 'สต๊อกตามผู้ช่วยดูแล' : 'สต๊อกตามผู้ดูแลหลัก';
  const roleLabel = myStockScope === 'assistant' ? 'ผู้ช่วยดูแล' : 'ผู้ดูแลหลัก';
  const filterPanel = adminMode ? `<section class="card admin-owner-filter"><div><p class="eyebrow">Admin view</p><h3>เลือกเจ้าหน้าที่ที่ต้องการดู</h3><p>หน้านี้แสดงเฉพาะรายการที่บุคคลนั้นเป็น <strong>${roleLabel}</strong> เพื่อไม่ปะปนหน้าที่หลักกับงานช่วยดูแล</p></div><div class="admin-owner-filter-controls single"><label>เจ้าหน้าที่<select id="myStockAdminOwner"><option value="">กรุณาเลือกชื่อ</option><option value="all" ${ownerParam==='all'?'selected':''}>ทุกคน</option>${ownerEntries.map(([email,name])=>`<option value="${esc(email)}" ${email===ownerParam?'selected':''}>${esc(name)}</option>`).join('')}</select></label></div></section>` : '';
  const tabBar = myStockScope === 'assistant'
    ? `<div class="my-stock-tabs"><button data-my-stock-tab="overview" class="${myStockTab==='overview'?'active':''}">ภาพรวมงานช่วยดูแล</button><button data-my-stock-tab="reorder" class="${myStockTab==='reorder'?'active':''}">ช่วยเตือนต้องเบิก <span>${reorder.length}</span></button></div>`
    : `<div class="my-stock-tabs"><button data-my-stock-tab="overview" class="${myStockTab==='overview'?'active':''}">ภาพรวม</button><button data-my-stock-tab="reorder" class="${myStockTab==='reorder'?'active':''}">ต้องเบิก <span>${reorder.length}</span></button><button data-my-stock-tab="settings" class="${myStockTab==='settings'?'active':''}">ตั้งค่าการเตือน</button></div>`;
  const expiredPending = groups.reduce((sum,g)=>sum+Number(g.expired_pending_lots||0),0);
  const nearExpiry = groups.filter(g=>isNearExpiryDate(g.nearest_expiry, expiryAlertDays(g))).length;
  const summary = `<div class="my-stock-kpis"><div><span>${myStockScope==='assistant'?'ช่วยดูแล':'ดูแลหลัก'}</span><strong>${groups.length}</strong><small>รายการ</small></div><div><span>${myStockScope==='assistant'?'ช่วยเตือนต้องเบิก':'ต้องเบิก'}</span><strong>${reorder.length}</strong><small>รายการ</small></div><div><span>ใกล้หมดอายุ</span><strong>${nearExpiry}</strong><small>รายการ</small></div><div><span>หมดอายุรอนำออก</span><strong>${expiredPending}</strong><small>Lot</small></div></div>`;
...
async function renderStock(initialFilter = 'select') {
  ...
  const draw = () => {
    ...
    if (filter && filter!=='all') arr=arr.filter(g=>filter==='expiry' ? isNearExpiryDate(g.nearest_expiry, expiryAlertDays(g)) : materialGroupStatus(g).key===filter);
...
const OPEN_LABEL_PERIODS = [
  ...
  ['3_MONTHS','3 เดือน'],
  ['6_MONTHS','6 เดือน'],
  ['CUSTOM','ระบุวันใช้ได้ถึงเอง']
];
function openLabelPeriodLabel(value) {
...
function openLabelPeriodShort(value) {
  return ({'MANUFACTURER_EXPIRY':'EXP ผู้ผลิต','24_HOURS':'24 ชม.','7_DAYS':'7 วัน','28_DAYS':'28 วัน','1_MONTH':'1 เดือน','3_MONTHS':'3 เดือน','6_MONTHS':'6 เดือน','CUSTOM':'กำหนดเอง'})[String(value || '').toUpperCase()] || 'กำหนดเอง';
}
function openLabelPeriodOptions(selected = 'CUSTOM') {
  const current = String(selected || 'CUSTOM').toUpperCase();
  return OPEN_LABEL_PERIODS.map(([value,label]) => `<option value="${value}" ${value===current?'selected':''}>${label}</option>`).join('');
}
...
function manufacturerExpiryLimit(value) {
...
function calculateOpenLabelUseUntil({openedAt, period, manufacturerExpiry = '', customUseUntil = ''}) {
  ...
  else if (code === '7_DAYS') calculated = new Date(openedDate.getTime() + 7*24*60*60*1000);
  ...
  else if (code === '6_MONTHS') calculated = addCalendarMonthsClamped(openedDate,6);
...
function formatThaiOpenDateTime(value, longYear = false) {
  const date = value instanceof Date ? value : new Date(value);
  if (Number.isNaN(date.getTime())) return 'ไม่ระบุ';
  return date.toLocaleString('th-TH', {
    day:'2-digit',month:'2-digit',year:longYear?'numeric':'2-digit',
    hour:'2-digit',minute:'2-digit',hour12:false
  }).replace(',', '');
}
function openLabelCalculationText(result) {
...
function renderUsageResult(material, rows, fromDate, toDate, summary) {
  ...
  const adjustmentOutRows=rows.filter(x=>x.tx_type==='ADJUSTMENT' && Number(x.quantity_delta||0)<0);
  const usageOutRows=[...issueRows,...adjustmentOutRows];
  ...
  const used=usageOutRows.reduce((s,x)=>s+Math.abs(Number(x.quantity_delta||0)),0);
  ...
  const map=new Map();
  rows.forEach(x=>{
    const reference=transactionReferenceDate(x); const k=reference.slice(0,7); if(!k)return;
    if(!map.has(k))map.set(k,{receive:0,issue:0,expired:0});
    const v=map.get(k), delta=Number(x.quantity_delta||0);
    if(x.tx_type==='RECEIVE')v.receive+=Math.abs(delta);
    if(x.tx_type==='ISSUE' || (x.tx_type==='ADJUSTMENT' && delta<0))v.issue+=Math.abs(delta);
    if(x.tx_type==='EXPIRED')v.expired+=Math.abs(delta);
  });
  ...
  return `<div class="usage-summary-head"><div><h3>${esc(material.name)}</h3><p>${d(fromDate)} – ${d(toDate)} · ${days.toLocaleString('th-TH')} วัน</p></div><button class="mini ghost" data-material-detail="${esc(material.code)}">ดูสต๊อกปัจจุบัน</button></div><div class="usage-kpi-grid usage-kpi-grid-7"><div class="usage-kpi current"><span>คงเหลือปัจจุบัน</span><strong>${qty(summary?.total_balance || 0)}</strong><small>${esc(material.unit || '')}</small></div><div class="usage-kpi receive"><span>นำเข้าช่วงนี้</span><strong>${qty(received)}</strong><small>${esc(material.unit || '')} · ${receiveRows.length} ครั้ง</small></div><div class="usage-kpi issue"><span>นำออก/ใช้ช่วงนี้</span><strong>${qty(used)}</strong><small>${esc(material.unit || '')} · ${usageOutRows.length} ครั้ง</small></div><div class="usage-kpi expired"><span>หมดอายุตามวัน EXP</span><strong>${qty(expired)}</strong><small>${esc(material.unit || '')} · ${expiredRows.length} ครั้ง</small></div><div class="usage-kpi"><span>เฉลี่ยต่อสัปดาห์</span><strong>${qty(avgWeek)}</strong><small>${esc(material.unit || '')}/สัปดาห์</small></div><div class="usage-kpi"><span>เฉลี่ยต่อเดือน</span><strong>${qty(avgMonth)}</strong><small>${esc(material.unit || '')}/เดือน</small></div><div class="usage-kpi"><span>เฉลี่ยต่อปี</span><strong>${qty(avgYear)}</strong><small>${esc(material.unit || '')}/ปี</small></div></div><div class="usage-grid" style="grid-template-columns:1fr"><section class="card usage-chart-card"><div class="section-title compact"><div><h3>นำเข้า–นำออก–หมดอายุ รายเดือน</h3><p class="muted small">นำออกรวมยอดปรับลดจากการตรวจสต๊อก · หมดอายุตามวัน EXP</p></div><div class="chart-legend"><span class="receive">นำเข้า</span><span class="issue">นำออก</span><span class="expired">หมดอายุ</span></div></div><div class="usage-bars">${months.map(([k,v])=>`<div class="usage-bar-row"><span>${usageMonthLabel(k)}</span><div class="bar-pair"><div class="bar-track"><i class="receive" style="width:${v.receive/max*100}%"></i></div><div class="bar-track"><i class="issue" style="width:${v.issue/max*100}%"></i></div><div class="bar-track"><i class="expired" style="width:${v.expired/max*100}%"></i></div></div><div class="bar-values three"><b>+${qty(v.receive)}</b><b>-${qty(v.issue)}</b><b>หมด ${qty(v.expired)}</b></div></div>`).join('') || '<div class="empty">ไม่มีรายการในช่วงวันที่นี้</div>'}</div></section></div><section class="card usage-history-card"><div class="section-title compact"><div><h3>ประวัติของสินค้านี้</h3><p class="muted small">${rows.length.toLocaleString('th-TH')} รายการในช่วงที่เลือก</p></div></div><div class="table-wrap"><table class="data-table"><thead><tr><th>วันเวลา</th><th>ประเภท</th><th>Lot</th><th>เปลี่ยนแปลง</th><th>ยอดหลังทำ</th><th>ผู้บันทึก</th></tr></thead><tbody>${rows.slice(0,300).map(x=>`<tr><td>${x.tx_type==='EXPIRED'&&x.expiry_date?`<span class="usage-expiry-date"><strong>EXP ${d(x.expiry_date)}</strong><small>ยืนยันนำออก ${dt(x.created_at)}</small></span>`:dt(x.created_at)}</td><td>${x.tx_type==='RECEIVE'?'<span class="badge ok">นำเข้า</span>':x.tx_type==='ISSUE'?'<span class="badge info">นำออก</span>':x.tx_type==='EXPIRED'?'<span class="badge danger">หมดอายุ</span>':x.tx_type==='ADJUSTMENT'?'<span class="badge warn">ปรับยอด</span>':`<span class="badge warn">${esc(x.tx_type)}</span>`}</td><td>${esc(x.lot_key)}</td><td class="${x.tx_type==='ISSUE'||x.tx_type==='EXPIRED'||Number(x.quantity_delta||0)<0?'negative-text':'positive-text'}">${x.tx_type==='RECEIVE'?'+':''}${qty(x.quantity_delta)} ${esc(x.unit)}</td><td>${qty(x.quantity_after)}</td><td>${esc(x.created_by_name || x.created_by_email || 'SYSTEM')}</td></tr>`).join('') || '<tr><td colspan="6" class="empty">ไม่มีข้อมูล</td></tr>'}</tbody></table></div>${rows.length>300?'<p class="field-hint">ตารางแสดง 300 รายการล่าสุด แต่ค่ารวมคำนวณจากข้อมูลทั้งหมด</p>':''}</section>`;
}
...
async function launchOpenLabelPrint(l, {
  popup = null,
  openedAt = '',
  openedBy = '',
  useUntilAt = '',
  ...
  const openedDate = openedAt instanceof Date ? openedAt : (parseLocalDateTime(openedAt) || new Date(openedAt));
  const useUntilDate = useUntilAt instanceof Date ? useUntilAt : (parseLocalDateTime(useUntilAt) || new Date(useUntilAt));
  if (Number.isNaN(openedDate?.getTime?.()) || Number.isNaN(useUntilDate?.getTime?.())) {
    try { printWindow.close(); } catch (_) {}
    toast('วันเปิดใช้หรือวันใช้ได้ถึงไม่ถูกต้อง', true);
    return false;
  }
  ...
  try {
    ...
    (async () => {
      try {
        ...
        try {
          const result = await sb.rpc('fn_log_open_label_print_v2', {
            p_issue_transaction_id:issueTransactionId || null,
            p_lot_id:l.lot_id || null,
            p_material_code:code || null,
            p_lot_no:lot === '-' ? null : lot,
            p_opened_on:dateInputValue(openedDate),
            p_expiry_date:dateInputValue(useUntilDate),
            p_opened_by:operator,
            p_source:source
          });
          if (result.error) throw result.error;
...
function openLabelIssueCard(row) {
  ...
  const periodBadge = row.open_label_default_period ? `<span class="badge info">${esc(openLabelPeriodShort(row.open_label_default_period))}</span>` : '';
  const status = printed
  ...
  return `<article class="open-label-issue-card" data-open-label-issue-card="${esc(row.transaction_id)}">
    <div class="open-label-issue-main">
      <div class="open-label-issue-title">
        <div><strong>${esc(row.material_name || row.label_name || row.material_code || '-')}</strong>${requiredBadge}${periodBadge}<p><span class="code-pill">${esc(`${row.material_code}-${row.lot_no}`)}</span> · นำออก ${dt(row.created_at)}</p></div>
        <div class="label-print-status">${status}</div>
      </div>
      <div class="open-label-issue-meta">
        <span>Lot <b>${esc(row.lot_no || '-')}</b></span>
        <span>EXP <b>${d(row.expiry_date)}</b></span>
        <span>ผู้นำออก <b>${esc(row.issued_by_name || row.issued_by_email || '-')}</b></span>
      </div>
    </div>
    <div class="open-label-issue-action">
      <button type="button" class="primary" data-open-label-print="${esc(row.transaction_id)}">${icon('print')} พิมพ์วันเปิดใช้</button>
    </div>
  </article>`;
}

function drawOpenLabelIssueRows(rows, containerId, emptyText) {
  const container = $(`#${containerId}`);
  if (!container) return;
  container.innerHTML = rows.length
    ? rows.map(openLabelIssueCard).join('')
    : `<div class="card empty">${icon('print')}<div>${esc(emptyText)}</div></div>`;
}

async function printOpenLabelIssue(transactionId) {
  ...
  openModal(`<section class="open-label-setup-modal"><div class="section-title compact"><div><p class="eyebrow">Opened reagent</p><h3>กำหนดวันเปิดใช้</h3><p class="muted small">ตรวจข้อมูลจากภาชนะจริงก่อนยืนยันพิมพ์</p></div></div>
    <div class="selected-lot"><div><strong>${esc(row.material_name || row.label_name || row.material_code || '-')}</strong><small>${esc(row.material_code || '')} · Lot ${esc(row.lot_no || '-')}</small></div><span class="badge info">EXP ${d(row.expiry_date)}</span></div>
    <form id="openLabelSetupForm" class="form-grid">
      <label>วันหมดอายุจากผู้ผลิต<input value="${esc(row.expiry_date ? d(row.expiry_date) : 'ไม่ระบุ')}" readonly></label>
      <label>วันและเวลาเปิดใช้<input id="openLabelSetupOpenedAt" type="datetime-local" value="${openedDefault}" required></label>
      <label>อายุการใช้งานหลังเปิด<select id="openLabelSetupPeriod">${openLabelPeriodOptions(defaultPeriod)}</select></label>
  ...
  const customWrap = $('#openLabelSetupCustomWrap');
  const resultEl = $('#openLabelSetupResult');
  const warningEl = $('#openLabelSetupWarning');
  const calculate = () => {
    const isCustom = periodInput.value === 'CUSTOM';
    customWrap.classList.toggle('hidden', !isCustom);
    const result = calculateOpenLabelUseUntil({
      openedAt:$('#openLabelSetupOpenedAt').value,
      period:periodInput.value,
      manufacturerExpiry:row.expiry_date || '',
      customUseUntil:$('#openLabelSetupCustom').value
    });
  ...
  $('#openLabelSetupForm')?.addEventListener('submit', async e => {
    ...
    button.disabled = true;
    const ok = await launchOpenLabelPrint(row, {
      popup,
      openedAt:result.openedDate,
      openedBy:profile?.display_name || profile?.email,
      useUntilAt:result.useUntil,
      manufacturerExpiry:row.expiry_date || '',
      period:result.period,
      issueTransactionId:row.transaction_id,
    ...
    if (!ok) return;
    closeModal();
    row.print_count = Number(row.print_count || 0) + 1;
    row.last_printed_at = new Date().toISOString();
    const card = document.querySelector(`[data-open-label-issue-card="${CSS.escape(String(transactionId))}"]`);
    if (card) card.outerHTML = openLabelIssueCard(row);
    toast('กำลังเปิดหน้าพิมพ์สติ๊กเกอร์วันเปิดใช้');
  });
}
...
async function renderOpenLabels() {
  ...
  const operatorSelect = $('#openLabelHistoryOperator');
  operatorSelect.insertAdjacentHTML('beforeend', (staffRes.data || []).map(x => `<option value="${esc(x.email)}">${esc(x.display_name)}</option>`).join(''));

  $$('[data-open-label-tab]').forEach(button => button.addEventListener('click', () => {
    const tab = button.dataset.openLabelTab;
    $$('[data-open-label-tab]').forEach(x => x.classList.toggle('active', x === button));
    $('#openLabelTodayPanel').classList.toggle('hidden', tab !== 'today');
    $('#openLabelHistoryPanel').classList.toggle('hidden', tab !== 'history');
  }));

  $('#searchOpenLabelHistory')?.addEventListener('click', async () => {
    const button = $('#searchOpenLabelHistory');
    const date = $('#openLabelHistoryDate').value;
    if (!date) return toast('กรุณาเลือกวันที่นำออก', true);
    button.disabled = true;
    button.innerHTML = `${icon('refresh')} กำลังค้นหา`;
    try {
      let query = sb.from('v_open_label_issue_queue').select('*').eq('issue_date',date).order('created_at',{ascending:false}).order('transaction_id',{ascending:false}).limit(1000);
      const material = $('#openLabelHistoryMaterial').value;
      const operator = $('#openLabelHistoryOperator').value;
      if (material) query = query.eq('material_code',material);
      if (operator) query = query.eq('issued_by_email',operator);
      const {data,error} = await query;
      if (error) throw error;
      const historyRows = data || [];
      const merged = new Map(openLabelIssueRowsCache.map(x => [String(x.transaction_id),x]));
      historyRows.forEach(x => merged.set(String(x.transaction_id),x));
      openLabelIssueRowsCache = [...merged.values()];
      drawOpenLabelIssueRows(historyRows, 'openLabelHistoryList', 'ไม่พบรายการตามวันที่และตัวกรองที่เลือก');
    } catch (e) {
      toast(errMsg(e), true);
    } finally {
      button.disabled = false;
      button.innerHTML = `${icon('search')} ค้นหา`;
    }
  });
  $('#refreshOpenLabelQueue')?.addEventListener('click', () => navigate('open-labels'));
}

async function renderOpenLabelCreate() {
  ...
  <section class="card open-label-generator-card"><div class="open-label-generator-head">${icon('print')}<div><h3>ข้อมูลบนสติ๊กเกอร์</h3><p>ขนาด 25 × 20 mm ตามแบบ FM-CNPL-034 Rev.00</p></div></div><form id="openLabelGeneratorForm" class="form-grid open-label-generator-form">
    ...
    <label>อายุการใช้งานหลังเปิด<select id="openLabelPeriod">${openLabelPeriodOptions('CUSTOM')}</select></label>
  ...
  const calculate = () => {
    ...
    $('#openLabelCustomWrap').classList.toggle('hidden', period !== 'CUSTOM');
    const result = calculateOpenLabelUseUntil({
      openedAt:$('#openLabelOpenedAt').value,
      period,
      manufacturerExpiry:$('#openLabelManufacturerExpiry').value,
      customUseUntil:$('#openLabelCustomUseUntil').value
    });
    const resultEl = $('#openLabelCalculatedUntil');
  ...
  calculate();
  $('#openLabelGeneratorForm')?.addEventListener('submit', async e => {
    e.preventDefault();
    const materialCode = $('#openLabelMaterial').value;
    ...
    const button = e.submitter;
    button.disabled = true;
    const ok = await launchOpenLabelPrint({
      material_code:material.code,
      material_name:material.name,
      label_name:material.label_name,
      lot_no:lot,
      expiry_date:manufacturerExpiry
    }, {
      popup,
      openedAt:result.openedDate,
      openedBy:profile?.display_name || profile?.email,
...
function scannedLotStatus(l, summary) {
  ...
  if (summary?.needs_reorder) {
    ...
    return {label:reorderStatusLabel(summary), badge:'warn'};
  }
...
function weeklyAssistantEmail(x) {
  return String(x?.assistant_responsible_email || '').trim().toLowerCase();
}

function weeklyOwnedByMe(x) {
  const me = String(profile?.email || '').trim().toLowerCase();
  return weeklyOwnerEmail(x) === me || weeklyAssistantEmail(x) === me;
}
...
function weeklyCheckedSystemQty(x) {
  for (const value of [x?.balance_at_check, x?.system_qty, x?.current_balance]) {
    if (value !== null && value !== undefined && value !== '' && Number.isFinite(Number(value))) return Number(value);
  }
  return 0;
}

function weeklyCheckedActualQty(x) {
  const value = x?.actual_qty;
  return value !== null && value !== undefined && value !== '' && Number.isFinite(Number(value))
    ? Number(value)
    : weeklyCheckedSystemQty(x);
}
...
function weeklyItemCard(x) {
  ...
  const checkedSystem = weeklyCheckedSystemQty(x);
  const checkedActual = weeklyCheckedActualQty(x);
  ...
  const checkedSummary = x.checked_at
    ? `<div class="weekly-result-inline"><span><small>ยอดระบบตอนตรวจ</small><strong>${qty(checkedSystem)} ${esc(x.unit || '')}</strong></span><span><small>จำนวนจริง</small><strong>${qty(checkedActual)} ${esc(x.unit || '')}</strong></span><span><small>การเปลี่ยนยอด</small><strong>${Math.abs(checkedDiff) < 1e-9 ? 'ไม่เปลี่ยน' : `${checkedDiff > 0 ? '+' : '-'}${qty(Math.abs(checkedDiff))} ${esc(x.unit || '')}`}</strong></span></div>`
    : '';
  const checkerLine = x.checked_at
    ...
    : '';
  return `<div class="card check-card ${x.checked_at ? (x.result === 'ADJUSTED' ? 'mismatch' : x.result === 'EXPIRED_REMOVED' ? 'expired-done' : 'checked') : expired ? 'expired-card' : ''}"><div class="check-info"><div><strong>${esc(x.material_name)}</strong><div class="lot-meta">${esc(lotKey(x))} · คงเหลือปัจจุบัน ${qty(x.current_balance)} ${esc(x.unit)} · EXP ${d(x.expiry_date)}</div><div class="lot-meta weekly-owner-line"><span><small>ผู้ดูแลหลัก</small>${esc(x.responsible_name || '-')}</span><span><small>ผู้ช่วยดูแล</small>${esc(x.assistant_responsible_name || '-')}</span></div>${delegationLine}${expired ? '<div class="expired-callout">หมดอายุแล้ว: ไม่ต้องนับ ให้ตรวจว่านำออกจากพื้นที่จริงแล้ว</div>' : ''}${checkedSummary}${checkerLine}</div>${action}</div></div>`;
}
...
async function renderWeekly() {
  ...
  const primaryOwnerEntries=[...new Map(items.filter(i=>i.responsible_email).map(i=>[i.responsible_email,i.responsible_name || i.responsible_email])).entries()].sort((a,b)=>String(a[1]).localeCompare(String(b[1]), 'th'));
  ...
  let f = weeklyActiveFilter || 'pending';
  const ownerFilter = $('#weeklyOwnerFilter');
  const defaultOwnerFilter = isAdminMode() ? (weeklyAdminActingForEmail || 'all') : 'mine';
  const validOwnerValues = new Set([...ownerFilter.options].map(o=>o.value));
  ownerFilter.value = validOwnerValues.has(weeklyOwnerFilterValue) ? weeklyOwnerFilterValue : defaultOwnerFilter;
  weeklyOwnerFilterValue = ownerFilter.value;
  const draw = () => {
    let arr = items;
    const owner = ownerFilter.value;
    if (owner === 'mine') arr = arr.filter(x => weeklyOwnedByMe(x) || String(x.checked_by_email || '').toLowerCase() === String(profile.email || '').toLowerCase());
    else if (owner !== 'all') arr = arr.filter(x => ownerMatches(x, owner, 'both'));
    if (f === 'primary') arr = arr.filter(x => weeklyOwnerEmail(x) === String(profile.email || '').toLowerCase());
    if (f === 'assistant') arr = arr.filter(x => weeklyAssistantEmail(x) === String(profile.email || '').toLowerCase());
    ...
    if (f === 'pending') {
      arr = arr.filter(x => !x.checked_at);
      if (!isAdminMode()) {
        const me = String(profile.email || '').toLowerCase();
        arr = arr.filter(x => weeklyOwnerEmail(x) === me || (weeklyDelegatedEmail(x) === me && ['PENDING','ACCEPTED','ADMIN_ASSIGNED'].includes(String(x.delegation_status || '').toUpperCase())));
      }
    }
  ...
  ownerFilter.addEventListener('change', () => { weeklyOwnerFilterValue=ownerFilter.value; draw(); });
  $$('[data-wf]').forEach(b => b.addEventListener('click', () => { f=b.dataset.wf; weeklyActiveFilter=f; $$('[data-wf]').forEach(x=>x.classList.toggle('active',x===b)); draw(); }));
...
function weeklyActingContextMarkup(x) {
  ...
  const assistant = x?.assistant_responsible_name || x?.assistant_responsible_email || 'ยังไม่กำหนด';
  ...
  return `<div class="weekly-acting-context"><span><small>ผู้ดูแลหลัก</small><strong>${esc(owner)}</strong></span><span><small>ผู้ช่วยดูแล</small><strong>${esc(assistant)}</strong></span>${delegated}<span><small>ผู้ตรวจจริง</small><strong>${esc(actual)}</strong></span></div>`;
}

function weeklyResultActorsMarkup(x) {
  const owner = x?.responsible_name || x?.responsible_email || 'ยังไม่กำหนด';
  const assistant = x?.assistant_responsible_name || x?.assistant_responsible_email || 'ยังไม่กำหนด';
  const checker = x?.checked_by_name || x?.checked_by_email || '-';
  const delegated = weeklyDelegationActive(x) && (x?.delegated_to_name || x?.delegated_to_email)
    ? `<span><small>ผู้ได้รับมอบหมาย</small><strong>${esc(x.delegated_to_name || x.delegated_to_email)}</strong></span>` : '';
  return `<div class="weekly-acting-context weekly-result-actors"><span><small>ผู้ดูแลหลัก</small><strong>${esc(owner)}</strong></span><span><small>ผู้ช่วยดูแล</small><strong>${esc(assistant)}</strong></span>${delegated}<span><small>ผู้ตรวจจริง</small><strong>${esc(checker)}</strong></span></div>`;
}

function openWeeklyResult(id) {
  const x = (window._weeklyItems || []).find(i => i.item_id === id);
  if (!x || !x.checked_at) return;
  const systemQty = weeklyCheckedSystemQty(x);
  const actualQty = weeklyCheckedActualQty(x);
  const diff = actualQty - systemQty;
  const matched = Math.abs(diff) < 1e-9;
  const status = x.result === 'EXPIRED_REMOVED' ? 'นำ Lot หมดอายุออกแล้ว' : matched ? 'ยอดตรงกับระบบ' : 'ปรับยอดแล้ว';
  const statusClass = x.result === 'EXPIRED_REMOVED' ? 'danger' : matched ? 'matched' : 'changed';
  const changeText = x.result === 'EXPIRED_REMOVED'
    ? `ระบบตัดยอดจาก ${qty(systemQty)} เป็น ${qty(actualQty)} ${x.unit || ''} หลังยืนยันนำของหมดอายุออกจากพื้นที่จริง`
    : matched
      ? `ยอดระบบและจำนวนจริงเท่ากันที่ ${qty(actualQty)} ${x.unit || ''} จึงไม่มีการเปลี่ยนยอด`
      : `ระบบปรับยอดจาก ${qty(systemQty)} เป็น ${qty(actualQty)} ${x.unit || ''} (${diff > 0 ? 'เพิ่ม' : 'ลด'} ${qty(Math.abs(diff))} ${x.unit || ''})`;
  const reason = x.reason_code || (matched ? 'ยอดตรงกับระบบ — ไม่ต้องระบุเหตุผล' : x.result === 'EXPIRED_REMOVED' ? 'ยืนยันนำ Lot หมดอายุออกจากพื้นที่' : '-');
  const detail = x.reason_detail || '-';
  openModal(`<section class="weekly-result-modal"><div class="weekly-check-modal-head"><div><p class="eyebrow">Weekly stock check</p><h3>ผลการตรวจ Lot</h3><p>${esc(x.material_name)} · ${esc(lotKey(x))} · EXP ${d(x.expiry_date)}</p></div><span class="badge ${x.result === 'EXPIRED_REMOVED' ? 'danger' : matched ? 'ok' : 'warn'}">${esc(status)}</span></div>${weeklyResultActorsMarkup(x)}<div class="weekly-result-grid"><div><small>ยอดระบบขณะตรวจ</small><strong>${qty(systemQty)}</strong><span>${esc(x.unit || '')}</span></div><div><small>จำนวนจริง</small><strong>${qty(actualQty)}</strong><span>${esc(x.unit || '')}</span></div><div><small>ผลต่าง</small><strong>${matched ? '0' : `${diff > 0 ? '+' : '-'}${qty(Math.abs(diff))}`}</strong><span>${esc(x.unit || '')}</span></div><div><small>ยอดหลังบันทึก</small><strong>${qty(actualQty)}</strong><span>${esc(x.unit || '')}</span></div></div><div class="adjustment-preview ${statusClass}">${esc(changeText)}</div><div class="weekly-result-reason"><div><small>เหตุผล</small><strong>${esc(reason)}</strong></div><div><small>รายละเอียด</small><p>${esc(detail)}</p></div></div><div class="weekly-result-time">บันทึกโดย ${esc(x.checked_by_name || x.checked_by_email || '-')} · ${dt(x.checked_at)}</div></section>`);
}

function openCheck(id) {
  const x = (window._weeklyItems || []).find(i => i.item_id === id);
  if (!x) return;
  if (x.checked_at) return openWeeklyResult(id);
  if (!canHandleItem(x)) {
  ...
  openModal(`<section class="weekly-check-modal"><div class="weekly-check-modal-head"><div><p class="eyebrow">Weekly stock check</p><h3>ตรวจ Lot</h3><p>${esc(x.material_name)} · ${esc(lotKey(x))} · EXP ${d(x.expiry_date)}</p></div></div>${weeklyActingContextMarkup(x)}<div class="weekly-check-summary"><div><small>ยอดระบบ</small><strong>${qty(current)}</strong><span>${esc(x.unit || '')}</span></div><div><small>จำนวนจริง</small><strong id="actualQtyPreview">${qty(current)}</strong><span>${esc(x.unit || '')}</span></div></div><div class="check-adjustment-guide">${icon('check')}<div><strong>กรอกจำนวนที่นับได้จริง</strong><span>หน้าต่างนี้จะแสดงยอดระบบ จำนวนจริง ตัวอย่างการเปลี่ยนยอด เหตุผล และรายละเอียดก่อนบันทึกทุกครั้ง</span></div></div><form id="checkForm" class="form-grid"><label>จำนวนที่นับได้จริง<input id="actualQty" type="number" min="0" step="0.01" value="${current}" required inputmode="decimal"></label><div id="adjustmentPreview" class="adjustment-preview matched">ยอดตรงกับระบบ ยังไม่มีการปรับจำนวน</div><div id="reasonFields" class="form-grid check-reason-fields is-disabled"><div class="check-reason-heading"><strong>เหตุผลและรายละเอียด</strong><span id="reasonHint">ใช้เมื่อจำนวนจริงไม่ตรงกับยอดระบบ</span></div><label>เหตุผล<select id="reasonCode" disabled><option value="">เลือกเหตุผล</option><option>นำออกแล้วไม่ได้บันทึก</option><option>รับเข้าหรือนำออกผิดจำนวน</option><option>สูญหายหรือหาไม่พบ</option><option>ชำรุด</option><option>นับครั้งก่อนผิด</option><option>Lot หรือสติ๊กเกอร์ไม่ตรง</option><option>อื่น ๆ</option></select></label><label>รายละเอียด<textarea id="reasonDetail" rows="3" disabled placeholder="ระบุสิ่งที่ตรวจพบและการดำเนินการ"></textarea></label></div><button class="primary" type="submit">ยืนยันผลตรวจ</button></form></section>`);
  const toggle = () => {
    ...
    $('#actualQtyPreview').textContent=Number.isFinite(actual)?qty(actual):'-';
    ...
    if (!changed) {
      ...
      preview.textContent=`ระบบจะปรับยอดจาก ${qty(current)} เป็น ${qty(actual)} ${x.unit || ''} (${diff>0?'เพิ่ม':'ลด'} ${qty(Math.abs(diff))} ${x.unit || ''})`;
    }
  };
...
async function exportReport(kind) {
  ...
  if (kind === 'minimum-stock') {
    ...
    const rows = groups.map(g => {
      ...
      const shortage = ['MINIMUM','BELOW_MINIMUM'].includes(mode) ? Math.max(Number(g.min_qty || 0) - Number(g.total_balance || 0), 0) : '';
      const modeLabel = mode === 'MONTHLY' ? 'เตือนรอบเบิกรายเดือน' : mode === 'LAST_ITEM' ? 'กำลังใช้ชิ้นสุดท้าย' : mode === 'BELOW_MINIMUM' ? 'เมื่อน้อยกว่าขั้นต่ำเท่านั้น' : mode === 'NONE' ? 'ไม่เปิดแจ้งเตือน' : 'เมื่อเท่ากับหรือต่ำกว่าขั้นต่ำ';
      return [
        g.material_code,
        g.material_name,
    ...
    saveCsv(`CNMI_Inventory_Minimum_Stock_${stamp}.csv`, [[
      ...
      'รูปแบบแจ้งเตือน','วันที่เตือนรายเดือน','เตือนใกล้หมดอายุล่วงหน้า','ผู้ดูแล คนที่ 1','ผู้ดูแล คนที่ 2','สถานะ','จำนวน Lot ที่ใช้งาน','วันหมดอายุใกล้สุด'
    ], ...rows]);
...
function bindOpenLabelAdminFields(prefix, defaultPeriod = 'CUSTOM') {
  const check = $(`#${prefix}OpenLabel`);
  const select = $(`#${prefix}OpenPeriod`);
  if (!check || !select) return;
  if (defaultPeriod) select.value = defaultPeriod;
  const sync = () => {
    select.disabled = !check.checked;
    select.closest('label')?.classList.toggle('field-disabled', !check.checked);
  };
...
async function renderAdmin() {
  ...
  const tabButton = (key,label,detail,iconName) => `<button type="button" data-admin-tab="${key}" class="${adminTab===key?'active':''}">${icon(iconName)}<span><strong>${label}</strong><small>${detail}</small></span></button>`;
  page.innerHTML = `<div class="page-head"><div><h2>ตั้งค่าผู้ดูแลระบบ</h2><p class="muted small">เลือกเมนูย่อยก่อนทำงาน ไม่ต้องเลื่อนผ่านข้อมูลทั้งหมด</p></div><span class="badge info">โหมด Admin</span></div>
  <nav class="admin-subnav" aria-label="เมนูย่อยผู้ดูแลระบบ">
    ${tabButton('overview','ภาพรวม','โหมด Admin และภาระงาน','chart')}
    ${tabButton('users','ผู้ใช้งาน','สิทธิ์ Staff / Admin','user')}
    ${tabButton('materials','วัสดุและผู้ดูแล','เพิ่มวัสดุ Minimum EXP และผู้ดูแล','box')}
  </nav>
  <section data-admin-panel="overview" class="admin-panel ${adminTab==='overview'?'':'hidden'}">
    <section class="admin-note"><strong>การทำงาน 2 โหมด</strong><p>บัญชี Admin สามารถสลับเป็น “เจ้าหน้าที่” เพื่อทำงานเหมือนทุกคน หรือ “ผู้ดูแลระบบ” เมื่อต้องตั้งค่าและดูงานรวม</p></section>
  ...
  <section data-admin-panel="materials" class="admin-panel ${adminTab==='materials'?'':'hidden'}">
    ...
    <div class="table-wrap"><table class="data-table"><thead><tr><th>รหัส</th><th>ชื่อวัสดุ</th><th>Minimum</th><th>เตือน EXP</th><th>วันเปิดใช้</th><th>ผู้ดูแลหลัก</th><th>ผู้ช่วยดูแล</th><th>จัดการ</th></tr></thead><tbody id="adminMaterialBody"></tbody></table></div>
  </section>`;
  const switchTab = key => {
    adminTab = ['overview','users','materials'].includes(key) ? key : 'overview';
    $$('[data-admin-tab]').forEach(button => button.classList.toggle('active',button.dataset.adminTab===adminTab));
    $$('[data-admin-panel]').forEach(panel => panel.classList.toggle('hidden',panel.dataset.adminPanel!==adminTab));
  };
  $$('[data-admin-tab]').forEach(button => button.addEventListener('click',()=>switchTab(button.dataset.adminTab)));
  const drawMaterials = () => {
    const body = $('#adminMaterialBody');
    if (!body) return;
    const q = normalizeMaterialSearch($('#adminMaterialSearch')?.value || '');
    const arr = (m || []).filter(x => !q || normalizeMaterialSearch(`${x.code} ${x.name} ${x.responsible_email} ${x.assistant_responsible_email || ''}`).includes(q));
    body.innerHTML = arr.map(x => `<tr><td>${esc(x.code)}</td><td><strong>${esc(x.name)}</strong><div class="muted tiny">ชื่อบนสติ๊กเกอร์: ${esc(x.label_name || x.name)}</div></td><td>${qty(x.min_qty)} ${esc(x.unit)}</td><td>${expiryAlertLabel(x)}</td><td>${x.open_label_required?`<span class="badge ok">${esc(openLabelPeriodShort(x.open_label_default_period))}</span>`:'<span class="badge">ไม่ใช้</span>'}</td><td><select data-owner-code="${esc(x.code)}">${ownerOptions(s, x.responsible_email || '')}</select></td><td><select data-assistant-owner-code="${esc(x.code)}">${ownerOptions(s, x.assistant_responsible_email || '')}</select></td><td><button class="mini" data-edit-material="${esc(x.code)}">แก้ไข</button></td></tr>`).join('') || '<tr><td colspan="8">ไม่พบวัสดุ</td></tr>';
    bindAdminOwnerEvents();
    queueResponsiveTables(page);
  };
  $('#adminMaterialSearch')?.addEventListener('input', drawMaterials);
  $('#adminAddMaterial')?.addEventListener('click', openMaterialCreator);
...
function bindAdminOwnerEvents() {
  $$('[data-owner-code]').forEach(sel => sel.addEventListener('change', async () => {
    ...
    if (row?.assistant_responsible_email && sel.value === row.assistant_responsible_email) {
      sel.value = row.responsible_email || '';
...
function openMaterialCreator() {
  ...
  const suggestedCode = nextMaterialCode(window._adminMaterials || []);
  openModal(`<h3>เพิ่มวัสดุใหม่</h3><p class="muted small">กรอกข้อมูลหลักให้ครบก่อนนำวัสดุเข้าสต๊อกครั้งแรก</p><form id="newMaterialForm" class="form-grid">
    <label>รหัสวัสดุ<input id="newMatCode" maxlength="30" value="${esc(suggestedCode)}" placeholder="เช่น BB073" required></label>
    <label>ชื่อวัสดุ/น้ำยา<input id="newMatName" maxlength="160" required></label>
    <label>ชื่อบนสติ๊กเกอร์<input id="newMatLabel" maxlength="80" placeholder="เว้นว่างเพื่อใช้ชื่อวัสดุ"></label>
    <label>หน่วยนับ<input id="newMatUnit" maxlength="40" placeholder="เช่น ขวด, กล่อง, ชุด" required></label>
    <label>Minimum Stock<input id="newMatMin" type="number" min="0" step="0.01" value="0" required></label>
    <label>เกณฑ์เตือน EXP<select id="newMatExpiryAlert"><option value="7">1 สัปดาห์</option><option value="14">2 สัปดาห์</option><option value="30" selected>1 เดือน — รายการทั่วไป</option><option value="240">8 เดือน — ต้องเคลมหรือจัดการล่วงหน้า</option></select></label>
  ...
  $('#newMaterialForm')?.addEventListener('submit', async e => {
    e.preventDefault();
    const code = String($('#newMatCode').value || '').trim().toUpperCase();
    const name = $('#newMatName').value.trim();
    const labelName = $('#newMatLabel').value.trim() || name;
    const unit = $('#newMatUnit').value.trim();
    const owner = $('#newMatOwner').value || null;
    const assistant = $('#newMatAssistantOwner').value || null;
    if (!/^[A-Z0-9_-]+$/.test(code)) return toast('รหัสวัสดุใช้ได้เฉพาะ A-Z, 0-9, ขีดกลาง หรือขีดล่าง',true);
    if (!name || !unit) return toast('กรุณาระบุชื่อวัสดุและหน่วยนับ',true);
    if (owner && assistant && owner === assistant) return toast('ผู้ดูแลหลักกับผู้ช่วยดูแลต้องเป็นคนละคน',true);
    const button=e.submitter; button.disabled=true;
    const {error} = await sb.rpc('fn_admin_add_material',{
      p_code:code,p_name:name,p_label_name:labelName,p_unit:unit,p_min_qty:Number($('#newMatMin').value || 0),
      p_expiry_alert_days:Number($('#newMatExpiryAlert').value || 30),p_responsible_email:owner,p_assistant_responsible_email:assistant,
      p_open_label_required:Boolean($('#newMatOpenLabel').checked),p_open_label_default_period:$('#newMatOpenLabel').checked?$('#newMatOpenPeriod').value:null
    });
    button.disabled=false;
    if (error) return toast(errMsg(error),true);
...
async function openMaterialEditor(code) {
  ...
  $('#matForm').addEventListener('submit', async e => {
    ...
    const assistant=$('#matAssistantOwner').value || null;
    if (owner && assistant && owner===assistant) return toast('ผู้ดูแลหลักกับผู้ช่วยดูแลต้องเป็นคนละคน',true);
    const {error} = await sb.from('materials').update({
      ...
      open_label_default_period:$('#matOpenLabel')?.checked ? $('#matOpenPeriod').value : null,
...
function normalizeReagentBarcodeByName(name, barcode) {
  // Barcode is case-sensitive. Preserve exactly what Admin saved, trimming only outer spaces.
  return String(barcode || '').trim();
}

function reagentBarcodeCorrectionInfo(name, barcode) {
  const original=String(barcode || '').trim();
  return {original,corrected:original,changed:false};
}
...
function normalizeReagentDraft(parsed = {}) {
  ...
  return {
    ...
    validFrom:String(parsed?.validFrom || dateInputValue(new Date())),
...
function loadReagentGeneratorDraft() {
  try {
    ...
    return raw ? normalizeReagentDraft(JSON.parse(raw)) : reagentDefaultDraft();
  } catch (_) {
...
function reagentDateRange(row) {
  const from = row?.valid_from ? d(row.valid_from) : '-';
  const to = row?.valid_to ? d(row.valid_to) : 'ปัจจุบัน';
  return `${from} – ${to}`;
}
...
async function renderReagentSetAdmin() {
  ...
  const openEditor=(state=reagentDefaultDraft(),{sourceId='',editId='',title='สร้างชุดน้ำยาใหม่',hint='กรอกข้อมูลชุดและรายการน้ำยา แล้วบันทึกให้ Staff เลือกใช้'}={})=>{
    const normalized=normalizeReagentDraft(state);sourceSetId=sourceId;editingSetId=editId;editorRows=normalized.rows.length?normalized.rows:[reagentEmptyRow()];
    ...
    $('#reagentAdminSave').textContent=editingSetId ? 'บันทึกการแก้ไข' : 'บันทึกชุดน้ำยา';
  ...
  const closeEditor=()=>{$('#reagentAdminEditor').classList.add('hidden');sourceSetId='';editingSetId='';$('#reagentAdminSave').textContent='บันทึกชุดน้ำยา';};
  ...
  const showSelected=()=>{
    ...
    $('#reagentAdminEditSet').addEventListener('click',()=>openEditor({
      setName:selectedSet.set_name||'',instrument:selectedSet.instrument_name||'',category:selectedSet.category||'',validFrom:selectedSet.valid_from||dateInputValue(new Date()),validTo:selectedSet.valid_to||'',notes:selectedSet.notes||'',reserveRight:selectedSet.reserve_right!==false,rows:selectedItems
    },{editId:selectedSet.id,title:'แก้ไขชุดน้ำยา',hint:'ปรับข้อมูลของชุดนี้แล้วบันทึกทับชุดเดิมได้'}));
  ...
  $('#reagentAdminSave').addEventListener('click',async()=>{
    ...
    const rpcName=editingSetId ? 'fn_update_reagent_barcode_set' : 'fn_save_reagent_barcode_set';
    const payload=editingSetId ? {
      p_set_id:editingSetId,p_set_name:checked.setName,p_instrument_name:checked.instrument||null,p_category:checked.category||null,p_valid_from:checked.validFrom,
      p_valid_to:checked.validTo||null,p_notes:checked.notes||null,p_reserve_right:checked.reserveRight,p_items:checked.rows
    } : {
    ...
    localStorage.removeItem(REAGENT_GENERATOR_DRAFT_KEY);const wasEditing=Boolean(editingSetId);const savedId=data || editingSetId;closeEditor();toast(wasEditing ? 'บันทึกการแก้ไขชุดน้ำยาแล้ว' : 'บันทึกชุดน้ำยาแล้ว');await reloadSets(savedId);
  });
